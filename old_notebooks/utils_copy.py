...
def fillna_knn(df: pd.DataFrame, base: list or np.array, target: str, tuning_params: dict, n_jobs=-1):
    print("Target: ", target)
    whole = [target] + base
    ...
    scaler = MinMaxScaler().fit(X_target[base].values)
    
    Y = X_target[target]
    X = scaler.transform(X_target[base].values)
...
def fillna_knn_reg(df: pd.DataFrame, base: list or np.array, target: str, tuning_params: dict, n_jobs=-1):
    print('Target: ', target)
    ...
    tuned_model=GridSearchCV(model, tuning_params, scoring='neg_mean_squared_error', cv=5, verbose=0, n_jobs=n_jobs)
    tuned_model.fit(X_train_80, y_train_80)

    print ("Best Score: {:.4f}".format(-tuned_model.best_score_) )
    print ("Best Params: ", tuned_model.best_params_)
    test_mse= mean_squared_error(y_true = y_test, y_pred = tuned_model.predict(X_test) )
...
def plot_feature_importance(importance, names, model_type, limit=35):
    ...
    sns.barplot(x=fi_df['feature_importance'].iloc[:limit], y=fi_df['feature_names'].iloc[:limit])
    #Add chart labels
